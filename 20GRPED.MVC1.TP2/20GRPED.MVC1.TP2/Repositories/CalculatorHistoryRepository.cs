    ...
    public class CalculatorHistoryRepository
    {
        ...
        public IEnumerable<CalculatorModel> GetAll(string filter = null)
        {
            ...
            using (var sqlCommand = new SqlCommand(cmdText, sqlConnection)) //já faz o close
            {
                ...
                ReadCalculatorModel(history, sqlCommand);
            }

            return history;
        }

        internal IEnumerable<CalculatorModel> GetAllOfThisOperator(string operatorInput)
        {
            ...
            var cmdText = $"SELECT " +
                ...
                $"Hora as '{nameof(CalculatorModel.Hora)}' " +
                $"FROM CalculatorHistory " +
                $"{Environment.NewLine}WHERE Operator = @operator" +
                $"{Environment.NewLine}ORDER BY Hora DESC";
            ...
            using (var sqlCommand = new SqlCommand(cmdText, sqlConnection)) //já faz o close
            {
                ...
                sqlCommand.Parameters
                    .Add("@operator", SqlDbType.VarChar).Value = operatorInput;
        ...
        private static void ReadCalculatorModel(List<CalculatorModel> history, SqlCommand sqlCommand)
        {
